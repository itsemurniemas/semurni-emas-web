...
export type AlertType = "error" | "success" | "warning" | "info" | "neutral";

interface AlertDialogDrawerProps {
  open: boolean;
...
const AlertDialogDrawer: React.FC<AlertDialogDrawerProps> = ({
  open,
  setOpen,
  type = "error",
  title,
  ...
  const buttonVariant = getButtonVariant(type);
...
function AlertHeader({ config }: AlertIconProps) {
  if (!config.icon) {
    return null;
  }
...
function getAlertConfig(type: AlertType) {
  switch (type) {
    case "error":
      return {
        bgColor: "fill-red-100 dark:fill-red-500/15",
        iconColor: "fill-red-600 dark:fill-red-500",
    ...
    case "success":
      return {
        bgColor: "fill-green-100 dark:fill-green-500/15",
        iconColor: "fill-green-600 dark:fill-green-500",
    ...
    case "warning":
      return {
        bgColor: "fill-yellow-100 dark:fill-yellow-500/15",
        iconColor: "fill-yellow-600 dark:fill-yellow-500",
    ...
    case "info":
      return {
        bgColor: "fill-blue-100 dark:fill-blue-500/15",
        iconColor: "fill-blue-600 dark:fill-blue-500",
...
export default AlertDialogDrawer;
