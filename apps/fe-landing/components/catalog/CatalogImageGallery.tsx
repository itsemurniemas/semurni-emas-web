...
const CatalogImageGallery = ({ catalog }: CatalogImageGalleryProps) => {
  ...
  // Get catalog images
  const catalogImages = catalog.images.map((img) => img.image);
  ...
  }, [hasImages, catalogImages.length]);
  ...
  const prevImage = () => {
    ...
    setCurrentImageIndex(
      (prev) => (prev - 1 + catalogImages.length) % catalogImages.length,
    );
  };
        ...
        <div className="flex-1">
          ...
          >
            <Image
              ...
              src={catalogImages[currentImageIndex]!}
              alt={`Catalog main view`}
              fill
        ...
        <div className="relative">
          ...
          >
            <Image
              key={currentImageIndex}
              src={catalogImages[currentImageIndex]!}
              alt={`Catalog view ${currentImageIndex + 1}`}
              fill
              className="object-cover transition-opacity duration-500 group-hover:scale-105 select-none"
            />
          </div>
