...
import type { PaymentSelection, PaymentMethod } from "../types";

interface BuyPaymentMethodStepProps {
  selected: PaymentSelection | null;
  onSelect: (method: PaymentSelection) => void;
  total?: number;
}

const BuyPaymentMethodStep: React.FC<BuyPaymentMethodStepProps> = ({
  selected,
  onSelect,
  total = 0,
}) => {
  ...
  const handleBankDetailsChange = (
    index: number,
    field: "bankAccountNumber" | "bankName",
    value: string,
  ) => {
    const updated = [...selectedMethods];
    ...
    // Check if the current state is valid and update selection
    const totalAmount = updated.reduce((sum, item) => {
      return sum + (parseFloat(item.amount) || 0);
    }, 0);
    const allMethodsHaveAmount = updated.every(
    ...
    const allNonCashHaveDetails = updated.every(
      (item) =>
        item.method !== "NON_CASH" ||
        (item.bankName &&
          item.bankName.trim() !== "" &&
              ...
              <div className="grid grid-cols-1 gap-3">
                {selectedMethods.map((item, index) => (
                  ...
                  >
                    ...
                    {/* Bank Details for NON_CASH payments */}
                    {item.method === "NON_CASH" && (
                      <div className="mt-4 space-y-3 bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-200 dark:border-blue-800">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full mt-1 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-brand-400"
                          />
                        </div>
                        <div>
                          <Label className="text-sm font-medium text-gray-900 dark:text-white">
                ...
                <p className="text-xs text-yellow-800 dark:text-yellow-400 mt-1">
                  {selectedMethods.some(
                    ...
                    : selectedMethods.some(
                      ...
                      ? "Isi Nama Bank dan Nomor Rekening untuk metode Non Tunai"
...
export default BuyPaymentMethodStep;
