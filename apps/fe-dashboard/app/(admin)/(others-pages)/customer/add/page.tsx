...
import { useCreateCustomerViewModel } from "./useCreateCustomerViewModel";
import { useRouter } from "next/navigation";
import { toast } from "react-toastify";

const AddCustomerPage: React.FC = () => {
  const router = useRouter();
  const { handleSubmit, isLoading, error } = useCreateCustomerViewModel();
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Specific logic for phone field
    if (name === "phone") {
      // Only allow numbers and common prefixes (+, 0, 6)
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (isFormValid && !isLoading) {
      ...
      if (success) {
        ...
        toast.error(error || "Gagal menambahkan pelanggan");
      }
    }
  };
  ...
  return (
    <div>
      ...
      <ComponentCard title="Tambah Pelanggan Baru">
        <form autoComplete="off" className="space-y-6" onSubmit={onSubmit}>
          ...
          <div className="space-y-6">
            ...
            <DropzoneComponent
              ...
              description="PNG, JPG atau SVG (Wajib)"
              maxFiles={1}
              accept={{
                "image/jpeg": [".jpg", ".jpeg"],
                "image/png": [".png"],
