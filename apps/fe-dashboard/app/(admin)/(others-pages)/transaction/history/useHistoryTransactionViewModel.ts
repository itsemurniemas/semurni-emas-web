...
export const useHistoryTransactionViewModel = () => {
  const { user } = useAuth();
  ...
  const fetchTransactions = async (
    ...
    try {
      ...
      const response = await useCase.execute({
        ...
        status:
          status && status !== ""
            ? (status as "PENDING" | "REJECTED" | "DONE")
            : undefined,
      });
  ...
  const fetchBranches = async () => {
    ...
    try {
      ...
      setBranchListState(DataViewState.success(branches));
    } catch (error) {
  ...
  const goToPage = (
    ...
    if (page > 0 && page <= pagination.totalPages) {
      fetchTransactions(page, branchId, transactionType, status);
    }
  };
